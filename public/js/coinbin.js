$(document).ready(function () {
	...
	var explorer_tx = "https://explorer.nusacoin.org/ext/gettx/"
	var explorer_addr = "https://explorer.nusacoin.org/ext/getaddress/"
	var explorer_block = "https://explorer.nusacoin.org/api/getblock?hash="
	var custom_exp_tx = "https://explorer.nusacoin.org/tx/";
	var custom_raw_tx = "https://explorer.nusacoin.org/api/getrawtransaction/";
	var custom_unspent = "https://apinusa.taobot.org/utxos/";
	var custom_history = "https://explorer.nusacoin.org/address/";
		...
		if (email.match(/[\s\w\d]+@[\s\w\d]+/g)) {
			if ($("#openPass").val().length >= 10) {
				if ($("#openPass").val() == $("#openPassConfirm").val()) {
					...
					$("#walletHistory").attr('href', custom_history + address);

					$("#walletQrCode").html("");
					var qrcode = new QRCode("walletQrCode");
					qrcode.makeCode("nusacoin:" + address);
	...
	$("#walletConfirmSend").click(function () {
		...
		tx.addUnspent($("#walletAddress").html(), function (data) {
			...
			if (dvalue >= total) {
				...
				//url: "http://paper.taobot.org:3002/api/sendrawtransaction?hex=" + $("#rawTransaction").val(),

				$.ajax({
					type: "POST",
					url: "https://apinusa.taobot.org/broadcast",
					//data: JSON.stringify({ "hex": $("#rawTransaction").val() }),
					data: { "raw": signed },
					dataType: "json",
					error: function (data) {
						var obj = $.parseJSON(data);
						var r = ' ';
						r += (obj.result) ? obj.result : '';
						r = (r != '') ? r : ' Failed to broadcast'; // build response 
						$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("Invalid SEND " + dvalue + " NUX unable to send " + total + " NUX").fadeOut().fadeIn();
						thisbtn.attr('disabled', true);
					},
					success: function (obj) {

						var data = obj;
						if ((data.id == 'curltext' || data.id == 'api-server') && data.result != null) {
							$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-success').html(' TXID: ' + data.result + '<br> <a href="https://explorer.taobot.org/tx/' + data.result + '" target="_blank">View on Blockchain Explorer</a>').fadeOut().fadeIn();
							walletBalance();
							thisbtn.attr('disabled', true);
						} else {
							$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("Unable send with message: " + data.error.message).fadeOut().fadeIn();
							thisbtn.attr('disabled', true);
						}
					}
				});
				// tx2.broadcast(function (data) {
				// 	if ($(data).find("id").text() == "curltext" && $(data).find("result").text() != "") {
				// 		$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-success').html('txid: <a href="' + custom_exp_tx + $(data).find("result").text() + '" target="_blank">' + $(data).find("result").text() + '</a>');
				// 		//$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(' TXID: ' + data.result + '<br> <a href="http://explorer.nusacoin.org/tx/' + data.result.txid + '" target="_blank">View on Blockchain Explorer</a>');
				// 	}/*
				// 	if ($(data).find("result").text() == "1") {
				// 		$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-success').html('txid: <a href="http://explorer.nusacoin.org/tx/' + $(data).find("txid").text() + '" target="_blank">' + $(data).find("txid").text() + '</a>');
				// 	}*/ else {
				// 		$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-danger').html(unescape($(data).find("message").text()).replace(/\+/g, ' '));
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of " + dvalue + " NUX unable to send " + total + " NUX").fadeOut().fadeIn();
				thisbtn.attr('disabled', false);
			}
	...
	function walletBalance() {
		if ($("#walletLoader").hasClass("hidden")) {
			...
			coinjs.addressBalance($("#walletAddress").html(), explorer_addr, function (data) {
				var obj = $.parseJSON(data);
				//console.log(obj.address + " and "+ obj.balance);
				if (obj.balance != 0 && obj.error != 'address not found.') {
					var v = obj.balance;
					$("#walletBalance").html(v + " NUX").attr('rel', v).fadeOut().fadeIn();
				} else {
					$("#walletBalance").html("0.00 NUX").attr('rel', v).fadeOut().fadeIn();
				}
	...
	$("#newSegWitKeysBtn").click(function () {
		...
		$("#typeAddress").val(sw.type);
		$("#newSegWitRedeemScript").val(sw.redeemscript);
		if (sw.type == 'bech32') {
			$("#newSegWitRedeemScript").val(sw.scriptpubkey);
		}
	...
	function scannerStart() {
		...
		if (navigator.getUserMedia) {
			...
			QCodeDecoder().decodeFromVideo(document.getElementById('videoReader'), function (er, data) {
				if (!er) {
					//var match = data.match(/^nusacoin\:([1|3|bc1][a-z0-9]{25,50})/i);
					var match = data.match(/^nusacoin:([13][1-9A-HJ-NP-Za-km-z]{25,33}|nu1[02-9ac-hj-np-z]{7,}|NU1[02-9AC-HJ-NP-Z]{7,})/);
	...
	function listUnspentTaobot(redeem) {
		$.ajax({
			type: "GET",
			url: custom_unspent + redeem.addr,
			//data: { 'address': `${redeem.addr}` },
			dataType: "json",
			error: function (data) {
				$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs!');
			},
			success: function (data) {
				if (data.result.success && data.error == null) {
					$("#redeemFromAddress").removeClass('hidden').html('<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="' + explorer_addr + redeem.addr + '" target="_blank">' + redeem.addr + '</a>');
					var all_info = data.result;
					var unspentList = all_info.unspents;
					if (unspentList.length == 0) {
						$("#redeemFromAddress").removeClass('hidden').html('<span class="glyphicon glyphicon-warning-sign"></span> Sorry, this address (<a href="' + explorer_addr + redeem.addr + '" target="_blank">' + redeem.addr + '</a>) has no balance');
					} else {
						for (var i in all_info.unspents) {
							var o = all_info.unspents[i];
	...
	function rawSubmitTaobot(thisbtn) {
		$(thisbtn).val('Please wait, loading...').attr('disabled', true);
		$.ajax({
			method: "POST",
			url: "https://apinusa.taobot.org/broadcast",
			//url: "https://explorer.nusacoin.org/api/sendrawtransaction?hex=" + $("#rawTransaction").val(),
			...
			success: function (data) {
				if ((data.id == 'curltext' || data.id == 'api-server') && data.result != null) {
					$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(' TXID: ' + data.result + '<br> <a href="' + custom_exp_tx + data.result + '" target="_blank">View on Blockchain Explorer</a>');
				} else {
					$("#rawTransactionStatus").addClass('alert-danger').removeClass('alert-success').removeClass("hidden").html(' Unexpected error, please try again.Reason : ' + data.error.message).prepend('<span class="glyphicon glyphicon-exclamation-sign"></span>');
				}
			},
			complete: function (data, status) {
				$("#rawTransactionStatus").fadeOut().fadeIn();
	...
	$("#verifyBtn").click(function () {
		...
		$("#verifyStatus").hide();
		$("#verifyTransactionDataOP").hide();
		if (!decodeRedeemScript()) {
			if (!decodeTransactionScript()) {
				if (!decodePrivKey()) {
					if (!decodePubKey()) {
						if (!decodeHDaddress()) {
							if (!decodeTXIDOpReturn(this)) {
	...
	function decodeRedeemScript() {
		...
		if (decode) {
			...
			if (decode.type == "multisig__") {
				...
				$("#verifyRsDataHodl .date").val(decode['checklocktimeverify'] >= 500000000 ? moment.unix(decode['checklocktimeverify']).format("DD/MM/YYYY HH:mm") : '-');
				$("#verifyRsDataHodl .dateunix").val(decode['checklocktimeverify']);
				$("#verifyRsData").removeClass("hidden");
	...
	function decodeTXIDOpReturn(btn) {
		var thisbtn = btn;
		var txid = $("#verifyScript").val()
		if (txid.length != 64) {
			return false;
		} else {
			$(thisbtn).val('Please wait, loading...').attr('disabled', true);
			$.ajax({
				type: "GET",
				url: custom_raw_tx + $("#verifyScript").val() + "/1",
				//data: JSON.stringify({ "hex": $("#rawTransaction").val() }),
				dataType: "json",
				error: function (data) {
				...
				complete: function (data, status) {
					...
					if (obj.result == null) {
						errorMsg = errorMsg + ' ' + obj.error.message;
						$("#verifyTransactionDataOP textarea").val(errorMsg);
						$("#verifyTransactionDataOP").removeClass('hidden').fadeIn();
						$(thisbtn).val('Submit').attr('disabled', false);
					} else {
						obj.result.vout.find(item => {
							if ((item.scriptPubKey.asm).startsWith('OP_RETURN')) {
								const input = item.scriptPubKey.asm;
								const [label, asm] = input.split(' ');
								dataascii = hexToUtf8(asm);
								//$("#verifyTransactionData").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(dataascii);
								$("#verifyTransactionDataOP textarea").val(dataascii);
								$("#verifyTransactionDataOP .txSize").html(dataascii.length);
								$("#verifyTransactionDataOP .blockSize").html(obj.result.confirmations != null ? obj.result.confirmations : 0);
								$("#verifyTransactionDataOP .timeSize").html(obj.result.blocktime != null ? moment.unix(obj.result.blocktime).format("DD/MM/YYYY HH:mm") : 0);
								$("#verifyTransactionDataOP").removeClass('hidden').fadeIn();
								$("#verifyTransactionDataOP .scriptOpUrl").val(document.location.origin + '' + document.location.pathname + '?verify=' + $("#verifyScript").val() + '#verify');
								$("#verifyTransactionDataOP #rawtextarea").val(obj.result.hex);
							}
						});
					}
					if (dataascii == '') {
						$("#verifyTransactionDataOP .blockSize").html(0);
						$("#verifyTransactionDataOP .timeSize").html(0);
						$("#verifyTransactionDataOP textarea").val(errorMsg);
						$("#verifyTransactionDataOP .txSize").html(0);
						$("#verifyTransactionDataOP .scriptOpUrl").val(0);
						$("#verifyTransactionDataOP #rawtextarea").val(obj.result.hex);
						$("#verifyTransactionDataOP").removeClass('hidden').fadeIn();
						//$("#verifyTransactionDataOP").removeClass('hidden').fadeOut().fadeIn();
						//$("#verifyTransactionData").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(errorMsg);
					}
	...
	function decodeTransactionScript() {
		...
		try {
			...
			$("#verifyTransactionData .transactionLockTime").html(decode['lock_time'] + ' (' + (decode['lock_time'] == 0 ? '0' : moment.unix(decode['lock_time']).format("DD/MM/YYYY HH:mm")) + ')');
			...
			$.each(decode.outs, function (i, o) {
				...
				if (o.script.chunks.length == 2 && o.script.chunks[0] == 106) { // OP_RETURN
					...
					h += '</tr>';
					h += '<tr>';
					h += '<td><input type="text" class="form-control" value="' + dataascii + '" readonly></td>';
					h += '</tr>';
				} else {
					var addr = '';
	...
	$(".qrcodeBtn").click(function () {
		...
		if (ta.length > 0) {
			...
			qrstr = "nusacoin:" + $('.address', thisbtn).val();
		}
	...
	function mathFees() {
		...
		if ((!isNaN($("#fees .feeSatByte:first").html())) && totalBytes > 10) {
			...
			$(".recommendedFee").html((0.000003).toFixed(8));
			$(".feeTxSize").html(0);
		}
	};
