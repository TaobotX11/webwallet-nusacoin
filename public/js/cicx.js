...
(function () {
	...
	coinjs.pub = 0x1c; //00
	coinjs.priv = 0x57; //80
	coinjs.multisig = 0x4b; //05
	coinjs.hdkey = { 'prv': 0x0488ade4, 'pub': 0x0488b21e };
	coinjs.bech32 = { 'charset': 'qpzry9x8gf2tvdw0s3jn54khce6mua7l', 'version': 0, 'hrp': 'ci' };

	coinjs.compressed = false;

	/* other vars */
	coinjs.developer = 'CdonasiJioJeCHKqSKjEqAaSR3ncNfE2fq'; //cicoin

	/* bit(coinb.in) api vars */
	coinjs.hostname = ((document.location.hostname.split(".")[(document.location.hostname.split(".")).length - 1]) == 'onion') ? 'coinbin3ravkwb24f7rmxx6w3snkjw45jhs5lxbh3yfeg3vpt6janwqd.onion' : 'explorer.taobot.org';
	...
	*/
	...
	coinjs.bech32Address = function (pubkey) {
		...
		return { 'address': address, 'type': 'bech32', 'redeemscript': Crypto.util.bytesToHex(program), 'scriptpubkey': Crypto.util.bytesToHex(keyhash) };
	}
	...
	coinjs.addressBalance = function (address, link, callback) {
		coinjs.ajax(link + address, callback, "GET");
	}
	...
	coinjs.hd = function (data) {
		...
		r.master = function (pass) {
			...
			var I = hasher.getHMAC("Cicoin seed", "TEXT", "SHA-512", "HEX");
	...
	coinjs.transaction = function () {
		...
		r.listUnspent = function (address, callback) {
			...
			coinjs.ajax('https://apicicoin.taobot.org:8443/api/get_utxos/', callback, "POST", ["address=" + address]);
		}
		...
		r.addUnspent = function (address, callback, script, segwit, sequence) {
			...
			this.listUnspent(address, function (data) {
				...
				//console.log(results.result.unspents);
				//return;
				//var unspent = xmlDoc.getElementsByTagName("unspents")[0];
				var unspent = results.result.unspents;
				if (unspent) {
					for (let i = 0; i < unspent.length; i++) {
						const el = unspent[i];
						var txhash = el.txid;
						var scr = script || el.scriptPubKey;
						var n = el.vout;
						...
						if (segwit) {
							...
							s.writeOp(0);
							s.writeBytes(coinjs.numToBytes(el.amount * 1, 8));
		...
		r.broadcast = function (callback, txhex) {
			...
			coinjs.ajax('https://apicicoin.taobot.org:8443/api/sendrawtransaction', callback, "POST", ["hex=" + tx]);
		}
